...
<template>
  <li
    ...
    <input
      ...
      :data-checked="checked_el == index" />
    <label
      class="h-min w-full p-1 text-sm text-aliceblue data-[checked=true]:font-bold data-[checked=true]:text-spacecadet data-[checked=false]:group-hover:font-bold data-[checked=false]:group-hover:text-saffron xl:text-lg"
      :data-checked="checked_el == index"
      :for="id">
      <slot></slot>
    </label>
